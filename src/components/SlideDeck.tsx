...
function GroupCard({group,index,reduceMotion}:{group:CoverGroup;index:number;reduceMotion:boolean}){
  ...
  return <motion.section className={`group-card group-card-${group.accent}`} aria-label={group.label} initial={{opacity:0,y:reduceMotion?0:16}} animate={{opacity:1,y:0}} transition={{duration:reduceMotion?.2:.5,delay}} whileHover={reduceMotion?undefined:{y:-4}}><motion.div className="group-accent-line" initial={{scaleX:reduceMotion?1:0}} animate={{scaleX:1}} transition={{duration:reduceMotion?.2:.55,delay:delay+.1}}/><header><span><Users/></span><h2>{group.label}</h2></header><ul>{group.members.map((member,row)=><motion.li key={member.ci} initial={{opacity:0,y:reduceMotion?0:10}} animate={{opacity:1,y:0}} transition={{duration:reduceMotion?.2:.35,delay:delay+.18+row*.05}}><strong>{member.name}</strong><code>C.I: {member.ci}</code></motion.li>)}</ul></motion.section>
}
