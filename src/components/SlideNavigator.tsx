...
export function SlideNavigator({ renderSlide }: {renderSlide:(index:number)=>ReactNode}) {
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(["ArrowRight"," ","PageDown"].includes(e.key)){e.preventDefault();go(index+1)}if(["ArrowLeft","PageUp"].includes(e.key)){e.preventDefault();go(index-1)}if(e.key.toLowerCase()==="f")document.documentElement.requestFullscreen?.()};const pop=()=>{const n=Number(location.hash.slice(1)||1)-1;if(Number.isFinite(n)){setDirection(n>=index?1:-1);setIndex(Math.max(0,Math.min(16,n)))}};window.addEventListener("keydown",key);window.addEventListener("popstate",pop);window.addEventListener("hashchange",pop);return()=>{window.removeEventListener("keydown",key);window.removeEventListener("popstate",pop);window.removeEventListener("hashchange",pop)}},[go,index]);
